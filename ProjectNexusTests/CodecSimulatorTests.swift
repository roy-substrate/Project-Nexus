...
final class CodecSimulatorTests: XCTestCase {
    ...
    func test_init_doesNotCrash_forAllTargets() {
        for target in CodecTarget.allCases {
            XCTAssertNoThrow(_ = CodecSimulator(codecTarget: target))
        }
    }

    func test_init_noneTarget_envelopeIsAllOnes() {
        let sim = CodecSimulator(codecTarget: .none, fftSize: 64, sampleRate: 48000)
    ...
    func test_applyToSpectrum_opus32k_attenuatesHighFrequencies() {
        let sim = CodecSimulator(codecTarget: .opus32k, fftSize: 1024, sampleRate: 48000)
    ...
    func test_applyToSpectrum_opus128k_nearTransparentAtMidRange() {
        let sim = CodecSimulator(codecTarget: .opus128k, fftSize: 1024, sampleRate: 48000)
        var spectrum = [Float](repeating: 1.0, count: 512)
        sim.applyToSpectrum(&spectrum)
    ...
    func test_applyToSpectrum_doesNotModifySpectrum_whenTargetIsNone() {
        let sim = CodecSimulator(codecTarget: .none)
        var spectrum = [Float](repeating: 0.5, count: 512)
        sim.applyToSpectrum(&spectrum)
        XCTAssertTrue(spectrum.allSatisfy { $0 == 0.5 })
    }

    func test_applyToSpectrum_outputLengthMatchesInput() {
        let sim = CodecSimulator(codecTarget: .opus64k, fftSize: 1024, sampleRate: 48000)
    ...
    func test_applyToSignal_outputLengthMatchesInput() {
        let sim = CodecSimulator(codecTarget: .opus64k, fftSize: 64, sampleRate: 48000)
        var signal = [Float](repeating: 0.5, count: 256)
        sim.applyToSignal(&signal)
        XCTAssertEqual(signal.count, 256)
    }

    func test_applyToSignal_noopWhenShorterThanFFTSize() {
        let sim = CodecSimulator(codecTarget: .opus64k, fftSize: 1024, sampleRate: 48000)
    ...
    func test_applyToSignal_noneTarget_isNoop() {
        let sim = CodecSimulator(codecTarget: .none, fftSize: 64, sampleRate: 48000)
    ...
    func test_applyToSignal_opus32k_vs_opus128k_differ() {
        let signal32k: [Float] = (0..<1024).map { Float.random(in: -0.5...0.5) }
        var s32 = signal32k
        var s128 = signal32k

        CodecSimulator(codecTarget: .opus32k,  fftSize: 64, sampleRate: 48000).applyToSignal(&s32)
        CodecSimulator(codecTarget: .opus128k, fftSize: 64, sampleRate: 48000).applyToSignal(&s128)
